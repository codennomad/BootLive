...
class TestYoutubeApi(unittest.TestCase):
    ...
    def test_get_live_chat_id_success(self, mock_build: MagicMock) -> None:
        """Tests getting a live chat ID successfully."""
        mock_youtube = MagicMock()
        mock_youtube.videos().list().execute.return_value = {
            "items": [{"liveStreamingDetails": {"activeLiveChatId": "test_chat_id"}}]
        }
        chat_id = get_live_chat_id(mock_youtube, "test_video_id")
        self.assertEqual(chat_id, "test_chat_id")

    @patch("youtube_api.build")
    def test_get_live_chat_id_no_video(self, mock_build: MagicMock) -> None:
    ...
    @patch("youtube_api.build")
    def test_get_live_chat_id_not_live(self, mock_build: MagicMock) -> None:
        """Tests getting a live chat ID for a video that is not a live stream."""
        mock_youtube = MagicMock()
        mock_youtube.videos().list().execute.return_value = {
            "items": [{"liveStreamingDetails": None}]
        }
        chat_id = get_live_chat_id(mock_youtube, "test_video_id")
        self.assertIsNone(chat_id)

    @patch("youtube_api.build")
    def test_send_chat_message(self, mock_build: MagicMock) -> None:
    ...
    @patch("youtube_api.build")
    def test_get_own_channel_name_success(self, mock_build: MagicMock) -> None:
        """Tests getting the bot's own channel name successfully."""
        mock_youtube = MagicMock()
        mock_youtube.channels().list().execute.return_value = {
            "items": [{"snippet": {"title": "Test Bot Channel"}}]
        }
        channel_name = get_own_channel_name(mock_youtube)
        self.assertEqual(channel_name, "Test Bot Channel")

    @patch("youtube_api.build")
    def test_get_own_channel_name_no_channel(self, mock_build: MagicMock) -> None:
