    ...
    if not credentials or not credentials.valid:
        if credentials and credentials.expired and credentials.refresh_token:
            ...
            if not os.path.exists(client_secrets_path):
                logging.error("client_secret.json not found. Please download it from the Google Cloud Console.")
                return None
...
def get_live_chat_id(youtube: Resource, video_id: str) -> Optional[str]:
    ...
    try:
        ...
        live_streaming_details: Optional[Dict[str, Any]] = response['items'][0].get('liveStreamingDetails')
        if not live_streaming_details:
            logging.error(f"Video with ID '{video_id}' is not a live stream or has no live chat.")
            return None
...
def get_chat_messages(youtube: Resource, live_chat_id: str, page_token: Optional[str] = None) -> Optional[Dict[str, Any]]:
...
def send_chat_message(youtube: Resource, live_chat_id: str, message_text: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logging.error(f"An HTTP error {e.resp.status} occurred while sending message: {e.content}")
        return None
...
def get_own_channel_name(youtube: Resource) -> Optional[str]:
    ...
    try:
        ...
        response: Dict[str, Any] = request.execute()
        if not response.get('items'):
            logging.warning("Could not retrieve authenticated user's channel information.")
            return None
